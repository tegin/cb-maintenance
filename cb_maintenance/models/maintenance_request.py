...
class MaintenanceRequest(models.Model):
    ...
    technician_user_id = fields.Many2one(
        domain="[('id', 'in', maintenance_team_id_member_ids)]", default=False
    )
    ...
    def write(self, vals):
        ...
        if "stage_id" in vals:
            self.filtered(lambda m: m.stage_id.done).write(
                {"close_date": fields.Date.today(), "solved": True}
            )
            self.filtered(lambda m: not m.stage_id.done).write(
                {"close_date": False, "solved": False}
            )
        return res
        ...
        for record in self:
            if record.stage_id.requires_technician and not (
                ...
                raise ValidationError(
                    _("To move forward you must first select a technician")
                )

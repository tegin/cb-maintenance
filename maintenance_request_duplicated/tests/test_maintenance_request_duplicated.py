...
class TestMaintenanceRequestDuplicated(TransactionCase):
    def setUp(self):
        super().setUp()
        self.team = self.env["maintenance.team"].create({"name": "Team 1"})
        self.m_request_1 = self.env["maintenance.request"].create(
            {"name": "Request 1", "maintenance_team_id": self.team.id}
        )
        self.m_request_2 = self.env["maintenance.request"].create(
            {"name": "Request 2", "maintenance_team_id": self.team.id}
        )
